...
$child_category_ids = [];
$belt_cat_result = $conn->query("SELECT category_id FROM categories WHERE parent_id = 3");
if ($belt_cat_result) {
    while ($cat = $belt_cat_result->fetch_assoc()) {
        $child_category_ids[] = (int)$cat['category_id'];
    }
}

// Nếu chọn category con thuộc parent_id=3 thì chỉ lấy sản phẩm thuộc category đó, còn lại lấy tất cả sản phẩm có parent_id=3
        ...
        <div class="col-12">
            <nav aria-label="Product pagination" class="mb-0">
                <ul class="pagination justify-content-center">
                    <?php if ($page > 1): ?>
                        <li class="page-item">
                            <a class="page-link" href="?page=<?php echo $page - 1; ?>&category=<?php echo $selected_category; ?>&sort=<?php echo $sort; ?>&price=<?php echo $price_range; ?>&color=<?php echo $color; ?>" aria-label="Previous">
                    ...
                    <?php if ($page < $total_pages): ?>
                        <li class="page-item">
                            <a class="page-link" href="?page=<?php echo $page + 1; ?>&category=<?php echo $selected_category; ?>&sort=<?php echo $sort; ?>&price=<?php echo $price_range; ?>&color=<?php echo $color; ?>" aria-label="Next">
